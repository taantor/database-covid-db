...
<body>
    ...
    function onEachFeature(feature, layer) {
        layer.on({
            ...
            click: function(e) {
                // Set the modal title to include the state name
                const stateName = feature.properties.name;
                $('#chartModalLabel').text(`${stateName} COVID-19 Data`);

                fetch(`chart_data.php?state=${stateName}`)
                    .then(response => {
                        if (!response.ok) {
                            throw new Error('Network response was not ok');
                        }
    ...
    info.update = function(props) {
        this._div.innerHTML = props ?
            `<h4>${props.name}</h4>FIPS: ${props.fips}<br />Cases: ${props.cases}<br />Deaths: ${props.deaths}` :
            'Hover over a state';
    };
